...
WCDB_IMPLEMENTATION(Person)//使用WCDB_IMPLEMENTATION宏在类文件定义绑定到数据库表的类。
WCDB_SYNTHESIZE(Person, localID)//使用WCDB_SYNTHESIZE宏在类文件定义需要绑定到数据库表的字段。
WCDB_SYNTHESIZE(Person, studentId)
WCDB_SYNTHESIZE(Person, name)
WCDB_SYNTHESIZE(Person, age)
WCDB_SYNTHESIZE(Person, createTime)
WCDB_SYNTHESIZE(Person, modifiedTime)
//WCDB_PRIMARY_AUTO_INCREMENT(Person, localID)//定义主键自增。
WCDB_PRIMARY(Person, localID)
WCDB_INDEX(Person, "_index", createTime)
@end
...
-(BOOL)insertData:(Person *)person {
    ...
    person.modifiedTime = [NSDate date];
    return [database insertOrReplaceObject:person into:TABLE_WCDB_NAME];
...
#pragma mark - 删
- (BOOL)deleteDataWithId:(NSString *)studentId {
    if (database == nil) {
        [self creatDatabase];
    }
...
-(NSArray<Person *> *)getData:(NSString *)studentId {
    if (database == nil) {
        [self creatDatabase];
    }
...
//    NSArray<Person *> * person = [database getObjectsOfClass:Person.class fromTable:TABLE_WCDB_NAME orderBy:Person.localID.order()];
    NSArray<Person *> * person = [database getObjectsOfClass:Person.class fromTable:TABLE_WCDB_NAME orderBy:Person.createTime.order()];
